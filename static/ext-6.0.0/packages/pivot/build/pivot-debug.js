...
Ext.define('Ext.pivot.dimension.Item', {
    ...
    getDefaultFormatRenderer: function (format) {
        ...
        return function (v) {
            ...
            return positive ? Ext.util.Format.number(v, frm) : '-' + Ext.util.Format.number(v, frm);
        };
    },
    /**
...
Ext.define('Ext.pivot.matrix.Remote', {
     ...
     */
    onRequestException: function (response) {
        Ext.MessageBox.show({
            title: 'Ошибка выполнения',
            msg: response.responseObject.data.error,
    ...
    delayedProcess: function () {
        ...
        if (ret === false) {
            ...
            params.totalid = me.totalid;
            Ext.Websocket.request({
...
Ext.define('Ext.pivot.plugin.configurator.DropZone', {
    ...
    onNodeDrop: function (node, dragZone, e, data) {
        ...
        var me = this,
                ...
                newCol, pos, newPos;
        // console.log(dragZone.panel.itemId, dropLocation.header.itemId);
        if ((dragZone.panel.itemId === "fieldsCt" && dropLocation.header.itemId === "fieldsLeftCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "fieldsCt" && dropLocation.header.itemId === "fieldsTopCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "dimensionsCt" && dropLocation.header.itemId === "fieldsAggCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "fieldsTopCt" && dropLocation.header.itemId === "fieldsAggCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "fieldsLeftCt" && dropLocation.header.itemId === "fieldsAggCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "fieldsAggCt" && dropLocation.header.itemId === "fieldsTopCt")) {
            return;
        }
        if ((dragZone.panel.itemId === "fieldsAggCt" && dropLocation.header.itemId === "fieldsLeftCt")) {
            return;
        }
        if (me.valid && dropLocation) {
            ...
            if (dragZone.id != me.panel.id) {
                ...
                if (!me.panel.isAgg) {
                    if (!(dragZone.panel.itemId === "fieldsCt" || dragZone.panel.itemId === "dimensionsCt")) {
                        dragZone.panel.remove(dragColumn);
                    }
                }
                if (!(me.panel.itemId === "fieldsCt" || me.panel.itemId === "dimensionsCt")) {
...
Ext.define('Ext.pivot.plugin.configurator.Panel', {
    ...
    panelAllDimensionsText: 'Drom Unused Dimensions Here',
    /**
    ...
    panelAllDimensionsTitle: 'Dimensions',
    /**
...
Ext.define('Ext.pivot.plugin.Configurator', {
    ...
    init: function (grid) {
        ...
        me.pivot = grid;
        grid.fields = me.fields = Ext.Array.from(me.fields);
        grid.totalFields = me.totalFields = Ext.Array.from(me.totalFields);
        grid.dimensionFields = me.dimensionFields = Ext.Array.from(me.dimensionFields);
